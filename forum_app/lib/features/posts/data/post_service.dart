...
class PostService {
  ...
  Future<Result<PaginatedResult<Post>>> fetchPosts({String? cursor, int limit = 10}) async {
    try {
      var query = _client
          .from('posts')
          .select('*, profiles(id, display_name, avatar_url, created_at), post_images(id, storage_path, position), comments(body, profiles(display_name), comment_images(id, storage_path, position))');
  ...
  Future<Result<void>> removeImages(List<String> imageIds) async {
    try {
